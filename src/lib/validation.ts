...
export const querySchemas = {
  ...
  gigs: z.object({ ...pagination, sort: z.enum(['createdAt', 'title', 'priceMinor', 'deliveryDays']).default('createdAt'), category: text.optional(), minPrice: integer.pipe(z.number().max(2147483647)).optional(), maxPrice: integer.pipe(z.number().max(2147483647)).optional() }).strict().refine(q => q.minPrice === undefined || q.maxPrice === undefined || q.minPrice <= q.maxPrice, { path: ['maxPrice'], message: 'maxPrice must be greater than or equal to minPrice' }),
